...
module.exports = function (config, log, since) {
  ...
  var q = async.queue(function (change, endTask) {
    ...
    var doc = change.doc
    var es_doc_url = config.elasticsearch + '/_doc/' + change.id
    if (config.key) es_doc_url = config.elasticsearch + '/_doc/' + doc[config.key]
  ...
  var feed = follow(follow_config, function (err, change) {
    ...
    if (q.length() > config.maxQueueSize) {
      feed.pause()
      console.log('pauing feed because maxQueueSize has been reached')
    }
  })
...
function handle_delete (config, es_doc_url, change, log, onDone, endTask) {
  ...
  if (config.urlTemplate) {
    tasks.push(function (cb) {
      recover(config.database, change.id, function (err, _prev_couch_doc) {
        if (err) _err = err
        _es_doc_url = run_compile(es_doc_url, _prev_couch_doc, log)
        cb()
      })
    })
  }
  ...
  async.series(tasks, function (err) {
    if (err) console.log('async error occured, but continuing', err)
